...
| 2026-04-19-late | **GR00T N1.6 expert_stack_with_vlm.onnx (DiT + state_encoder + action_encoder/decoder, 5 inputs)** vs PyTorch `GR00TFullStack.forward(noisy, t, pos, state, vlm_kv)` | **cos = +1.000000, max_abs = 1.78e-05 (fp32 noise)** | `modal run scripts/modal_gr00t_monolithic_export.py --vlm && modal run scripts/modal_gr00t_monolithic_export.py --vlm-parity`. Step 4a of the Eagle VLM export plan: ONNX version of the extended GR00TFullStack with state + vlm_kv as external inputs instead of zero-stubbed. 4.4GB total ONNX. Per-step wrapper parity (hand-rolled ref vs GR00TFullStack) was cos=+1.000000, max_abs=0.0 bit-exact (Step 3). The fp32 noise of 1.78e-05 in ONNX parity is equivalent to the existing GR00T zero-stub export (8.34e-07). | Modal runs `ap-v1tRMhS2ebUBoLgDp5wyBs` (export) + `ap-n7oJT5C20y4tEdVu4D55n1` (parity). Cost: ~5 min A100-40GB export + ~3 min A100-40GB parity. **Pending Step 4b**: Eagle VLM backbone ONNX (SigLIP + Qwen2 + mlp1). |
