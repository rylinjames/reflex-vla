...
| 2026-04-18 | **pi0 num_steps=10 monolithic ONNX** (12.5GB) vs PyTorch `PI0Pytorch.sample_actions(num_steps=10)` on shared seeded inputs — the canonical flow-matching denoise | **first-action cos = +0.977058, first-action max_abs = 1.31e-01; full-chunk max_abs = 2.48e-01** | `modal run scripts/modal_pi0_monolithic_export.py --parity --num-steps 10`. Export requires `create_causal_mask → None` shim to dodge the `835 -> 886` broadcast error during `torch.export` FakeTensor tracing (suffix-token push of K-length not handled by transformers 5.3's mask rebuild). Semantic cost: prefix pad positions are no longer masked out. Still strictly better than num_steps=1 (cos=0.897 vs same reference) and closer to what customers expect from pi0 canonical behavior. cos=1.0 at num_steps=10 requires deeper transformers patching — tracked for v0.3 under `num-steps-quality-gate`. | Modal run 17 (num_steps=10 breakthrough) |
