"""Modal: pi0.5 monolithic ONNX export. Same wrap pattern as pi0 + the
three 2026-04-19 patches (F.pad mask, frozen DynamicLayer.update, past_kv
seq_length for mask), reused unchanged — pi0.5's denoise_step is
structurally identical to pi0's. Only delta at the wrapper level: pi0.5
has no `state` input (state is tokenized into language tokens upstream).
...
app = modal.App("pi05-monolithic-export")
...
# Persistent volume for HF cache (pi05_base is ~14GB)
...
def export_pi0_monolithic_modal(
    ...
    _orig_embed = modeling_pi05.PaliGemmaWithExpertModel.embed_image
    ...
    modeling_pi05.PaliGemmaWithExpertModel.embed_image = _patched_embed_image
    ...
    from lerobot.policies.pi05 import modeling_pi05 as _mp0
    from lerobot.policies.pi05.modeling_pi05 import make_att_2d_masks as _make_att_2d_masks

    _patched_called = [0]

    def _patched_denoise_step(self, prefix_pad_masks, past_key_values, x_t, timestep):
        # pi0.5 signature: NO state arg (state is tokenized into language).
        _patched_called[0] += 1
    ...
    _mp0.PI05Pytorch.denoise_step = _patched_denoise_step
    ...
    _dstep_inner = _mp0.PI05Pytorch.denoise_step

    def _denoise_step_with_flag(self, prefix_pad_masks, past_key_values, x_t, timestep):
        # pi0.5 signature: NO state arg.
        _denoise_phase[0] = True
        try:
            return _dstep_inner(self, prefix_pad_masks, past_key_values, x_t, timestep)
        finally:
            _denoise_phase[0] = False

    _mp0.PI05Pytorch.denoise_step = _denoise_step_with_flag
    ...
    policy = PI05Policy.from_pretrained(model_id).eval().to("cpu").to(torch.float32)
    print(f"[modal] Loaded in {time.time() - t0:.1f}s")

    # ---- Build wrapper ----
    class Pi05MonolithicWrapper(nn.Module):
        def __init__(self, pi05_pytorch_model, num_steps=10):
            super().__init__()
            self.model = pi05_pytorch_model
        ...
        def forward(
            ...
            # pi0.5 has NO state input — state is tokenized into lang_tokens upstream.
    ...
    wrapper = Pi05MonolithicWrapper(policy.model, num_steps=num_steps).eval()
...
def parity_test_monolithic(model_id: str = "lerobot/pi05_base", num_steps: int = 1):
    ...
    # Build same dummy inputs as export (NO state for pi0.5)
    B = 1
    cfg = policy.config
    chunk = cfg.chunk_size
    action_dim = cfg.max_action_dim
