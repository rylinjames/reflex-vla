...
# Eagle VLM: export-only vendor (2026-04-19 by reflex-vla).
...
# peft — but we don't call them in export (use_backbone_lora=0,
# use_llm_lora=0 in the GR00T checkpoint config).
try:
    ...
    def get_peft_model(*args, **kwargs):
        raise ImportError(
            "peft is required for LoRA wrapping but not for export. "
            "Install peft if training; otherwise this path should not be called."
        )
    ...
    def __init__(self, config: Eagle25VLConfig, vision_model=None, language_model=None):
        ...
        if vision_model is not None:
            ...
            if config.vision_config.model_type == "siglip_vision_model":
                # Export mode: force eager (FA2 is not consumable by ONNX/TRT).
                # Keep FA2 as the upstream default for training if someone
                # explicitly passes it in the config.
                if getattr(config.vision_config, "_attn_implementation", "") in (
        ...
        if language_model is not None:
            ...
            if config.text_config.architectures[0] == "LlamaForCausalLM":
                ...
                # Export mode: override FA2 → eager. Upstream asserts FA2;
                # we relax that for ONNX/TRT export compatibility.
                if getattr(config.text_config, "_attn_implementation", "") == "flash_attention_2":
